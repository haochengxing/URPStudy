...
public class SSPRTexGenerator
{
    ...
    private int _kernelPass1;
    private int _kernelPass2;
    ...
    private void UpdateKernelIndex()
    {
        ...
        _kernelPass1 = _computeShader.FindKernel("DrawReflectionTex1");
        _kernelPass2 = _computeShader.FindKernel("DrawReflectionTex2");
    ...
    public bool excludeBackground{
        get { return _excludeBackground; }
        set { _excludeBackground = value;
            if (_computeShader)
            {
                UpdateKernelIndex();
            }
    ...
    public void Render(CommandBuffer cmd,ref RenderingData renderingData,ref PlanarDescriptor planarDescriptor)
    {
        ...
        cmd.SetComputeTextureParam(_computeShader, _kernelPass1, ShaderProperties.CameraColorTexture, cameraColorTex);
        cmd.SetComputeTextureParam(_computeShader, _kernelPass1, ShaderProperties.Result, _reflectionTexID);
        cmd.DispatchCompute(_computeShader, _kernelPass1, threadGroupX, threadGroupY, 1);

        cmd.SetComputeTextureParam(_computeShader, _kernelPass2, ShaderProperties.CameraColorTexture, cameraColorTex);
        cmd.SetComputeTextureParam(_computeShader, _kernelPass2, ShaderProperties.Result, _reflectionTexID);
        cmd.DispatchCompute(_computeShader, _kernelPass2,threadGroupX,threadGroupY,1);
