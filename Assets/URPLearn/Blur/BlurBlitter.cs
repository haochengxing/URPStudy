...
public class BlurBlitter
{
    ...
    public BlurType blurType
    {
        ...
        set {
            _blurType = value;
            _usingShader = FindShaderByBlurType(_blurType);
            if (!_usingShader)
            {
    ...
    {
        get { return _iteratorCount; }
        set { _iteratorCount = Mathf.Clamp(value, 0, 5); }
    }

    public int downSample
    {
        get { return _downSample; }
        set { _downSample = Mathf.Clamp(value, 1, 5); }
    }

    public int boxKernelSizeHalf
    {
        get { return _boxKernelSizeHalf; }
        set { _boxKernelSizeHalf = Mathf.Clamp(value,1,4); }
    }

    public float blurScale
    {
        get { return _blurScale; }
        set { _blurScale = Mathf.Clamp(value, 1, 5); }
    }
    ...
    public void Render(CommandBuffer cmd) {
        ...
        if (_downSample<=1)
        {
            if (_iteratorCount>0)
            {
                ...
                for (int i = 0; i < _iteratorCount; i++)
                {
